...
`research/evidence/V1_HYBRID_CRITICAL_ROOTS_PARTIAL.json` now holds 462 accepted float64 roots plus 77 independent Julia BigFloat escalations (canary plus harvest of run `31901007587` including chunk 22, plus local BigFloat cells 28/60/92). Frozen gates still hold (max accepted event `1.99e-8`, max closure `1.34e-9`). 81 cells remain missing. This is not 620/620 and not SOLVED.
