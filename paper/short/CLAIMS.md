...
| 2 | Sampled set **supported as** one continuation-connected component; 26 adversarial links crossed bidirectionally; chart independence on exactly 1 of 26; endpoints of the pathological bridge 0.0617 apart | `research/V1_CONNECTIVITY_CERTIFICATE_2026-08-15.json` / `.md` | bounded continuation support — 26 links of 135,444, chart independence on 1 of them; **not** a certification and not a theorem |
...
| 16 | Secondary-left birth is **not** a certified nondegenerate fold: transversality real (∂G/∂m1 = 28.045563274511, h-convergent) but the curvature is undetermined: stationarity PASSES (∂G/∂m2 → −6.10e-14 against a 1e-6 gate) while ∂²G/∂m2² diverges as h⁻², −3.9e8 → −2.5e10 over h = 8e-5…1e-5. The earlier "−30" was that second derivative scaled by 12h², a scaling artifact and not a test result | fold-geometry artifacts | **not claimed** |
...
| 20 | The BigFloat verifier never compares a located event against the frozen gate: `julia/verify_critical_points.jl` contains zero references to `max_abs_event`, `event_gate` or any equivalent, and refines only to a requested `target`/`width`. So "located" does not imply "inside the gate". **Correction:** earlier revisions of this row cited two SUCCESS rows at \|event\| 1.15e-7 and 2.27e-7. Those came from CI run 32078728694, whose artifacts are not committed to `research/evidence/`, and no committed artifact reproduces them — a search of every evidence JSON finds no pass-status row above 1e-7. The code claim stands on the source; the two numbers are withdrawn as uncited. | `julia/verify_critical_points.jl` | code defect, source-verified; the two numbers withdrawn |
