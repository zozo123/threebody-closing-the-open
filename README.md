...
> **Scientific status: OPEN.** Gate C passed (the catalog is supported as one continuation-connected component). Gate A passed for the mechanism words the paper is allowed to use (physical `+1`, opposite-Krein Hamiltonian–Hopf, three mixed `(4,4)` roots). Gate B (mechanism-resolved Floquet critical graph on that sheet) and Gate D (solved freeze) are pending. All 620 published S/U cells are localized once and assigned to seven mechanism polylines and twelve mixed germs are frozen, but four edge ends are still unclassified — so the assembled graph's edge incidence is two components, not one — and the two secondary endpoints, the daughter class, and the completeness freeze remain gated. "Completeness" is bounded and the bound is small: the frozen neck raster covers **0.0173%** of the declared mass domain (`scripts/completeness_scope.py`). This is not a solution of the general three-body problem, and no priority claim is made. See `research/OPEN_PROBLEM.md`, `research/NOVELTY_AUDIT.md`, and `paper/main.tex`.
