...
> **Scientific status: OPEN.** Gate C passed (the catalog is supported as one continuation-connected component). Gate A passed for the mechanism words the paper is allowed to use (physical `+1`, opposite-Krein Hamiltonian–Hopf, three mixed `(4,4)` roots). Gate B (mechanism-resolved Floquet critical graph on that sheet) and Gate D (solved freeze) are pending. All 620 published S/U cells are localized once and assigned to seven catalog polylines, six further event-sign polylines carry the label-invisible 2→1 / 1→2 curves the S/U census cannot see, and sixteen mixed germs are frozen. There are no unclassified edge ends and secondary-left birth is a projection fold, but the edge incidence is still three components and `sign_topology_clean` is still false: the loaded audits predate the supplemental edges. "Completeness" is now frozen, but bounded, and the bound is small: the frozen neck raster (`research/evidence/V1_NECK_RASTER_2026-08-16.json`, m1 ∈ [0.997, 0.999] × m2 ∈ [0.993, 1.012] at step 1e-4, 4011 samples, all 21 lines separated) covers **0.0253%** of the declared mass domain, and the only other completeness input — the 12-proposal off-grid active-learning screen — is not a sample of the rest of that domain but one pocket at the principal lower transition, m1 ∈ [0.80005, 0.80276] × m2 ∈ [0.75266, 0.75407], **0.002539%** of the domain, self-described as "AI proposals plus float64 screening only; not scientific discovery evidence". Both figures come from `scripts/completeness_scope.py`, which cross-checks them against the committed raster. This is not a solution of the general three-body problem, and no priority claim is made. See `research/OPEN_PROBLEM.md`, `research/NOVELTY_AUDIT.md`, and `paper/main.tex`.
