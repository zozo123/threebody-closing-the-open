...
> **Scientific status: OPEN.** Gate A, Gate B (defined conjuncts), and Gate C passed. The assembler has flipped `release_ready` on a 13-edge graph. Both 2026-08-17 sign-topology suites report `missing_critical_curve = 0`, `forbidden_component_flip = 0`, and no L-path face mismatch. Gate D is pending — novelty must be signed by the named human author on the freeze day — so SOLVED is forbidden. "Completeness" is frozen but bounded: the neck raster (`research/evidence/V1_NECK_RASTER_2026-08-16.json`, m1 ∈ [0.997, 0.999] × m2 ∈ [0.993, 1.012] at step 1e-4, 4011 samples, all 21 lines separated) covers **0.0253%** of the declared mass domain, and the only other completeness input — the 12-proposal off-grid active-learning screen — is not a sample of the rest of that domain but one pocket at the principal lower transition, m1 ∈ [0.80005, 0.80276] × m2 ∈ [0.75266, 0.75407], **0.002539%** of the domain, self-described as "AI proposals plus float64 screening only; not scientific discovery evidence". Both figures come from `scripts/completeness_scope.py`. This is not a solution of the general three-body problem, and no priority claim is made. See `research/OPEN_PROBLEM.md`, `research/NOVELTY_AUDIT.md`, and `paper/main.tex`.
