    ...
    for path in paths:
        ...
        for result in record.get("results", []) or []:
            ...
            if len(matches) != 1:
                ...
                continue
            endpoint = matches[0]["endpoints"][side]
            # A resolution is a WALK, and it must be checked at BOTH ends.
            #
            # The first cut checked only that the edge endpoint sat within the
            # organizer reach.  That is the right test for a germ, which lives
            # NEXT TO its organizer, and the wrong test for a continuation,
            # which deliberately walks away from the endpoint to find one.  It
            # rejected plus_one component 12's genuinely resolved low terminus
            # for sitting 6.01e-2 from secondary_right_death -- which is simply
            # how far the continuation travelled.
            #
            # So: the walk must START at the endpoint it claims to explain, and
            # END within reach of the node it claims to have reached.  Checking
            ...
            if not seed or not final:
                errors.append(
                    f"{path.name}: component {component} {side_word} records no "
                    "seed_masses/final_masses, so which terminus it walked from "
                    "cannot be established"
                )
                continue
            began_here = math.dist(
                [float(v) for v in list(seed)[:2]], [float(v) for v in list(here)[:2]]
            )
            if began_here > GERM_ATTACH_DISTANCE:
                errors.append(
                    f"{path.name}: component {component} {side_word} started "
                    f"{began_here:.4e} from this terminus, beyond "
                    f"{GERM_ATTACH_DISTANCE}; it did not walk from here"
                )
                continue
            recomputed = math.dist(
                [float(v) for v in list(final)[:2]],
                [float(v) for v in list(organizer)[:2]],
            )
            if recomputed > GERM_ATTACH_DISTANCE:
                errors.append(
                    f"{path.name}: component {component} {side_word} ended "
            ...
            if claimed is not None and not math.isclose(
                ...
                errors.append(
                    ...
                    f"miss_mass {float(claimed):.6e} but its walk ended "
                    f"{recomputed:.6e} from {node_id} -- the reported distance "
                    "was not measured where the walk stopped"
                )
                continue
            ...
            endpoint["walk_started_from_terminus"] = began_here
