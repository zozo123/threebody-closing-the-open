...
def endpoint_resolution_bindings(
    ...
    for path in paths:
        ...
        for result in record.get("results", []) or []:
            ...
            # (V1_SAMPLED_ENDPOINT_RESOLUTION_2026-08-17.json did exactly that
            # AS ORIGINALLY PRODUCED: its two component-12 results were
            # byte-identical, and the shared miss_mass 6.5263e-3 was the distance
            # from the `end` terminus, while `start` sits 2.3713e-2 away, 2.96x
            # beyond the reach.  The artifact has since been regenerated by the
            # fixed resolver and now records both sides as unresolved; this check
            # is what stands between that class of record and a binding.)
